  ...
  writeClimbToDatabase(BuildContext context, User user, String climber, String route, String style) async {
    ...
    if (compareStyles(style, bestStyle) >= 0){
      await ref.child(user.userID).update({
        ...
        "points": calculatePoints(points, getIsClimbThere(route, climber), climber, route),
      }).then((document) => showSnackBar(context, 'Climb added to the database!'));
    }
  }

  writeActivityToDatabase(BuildContext context, User user, String climber, String activity, num points) async {
    //if -1, then something went wrong!
...
calculateActivityPoints(num points, isActivityThere, String climber, String activityName) {
...
bool getBestActivity(String climberName, String activityName, num points) {
  if (climberName == results.climberOneActivities.climberName){
    num activityPointsBefore = results.climberOneActivities.getActivity(activityName).points;
    ...
    num activityPointsBefore = results.climberTwoActivities.getActivity(activityName).points;
    if (points > activityPointsBefore) {
      return true;
    } else {
      return false;
...
num calculatePoints(num points, bool isClimbThere, String climber, String routeName) {
  ClimbedPlaces climbedPlaces = results.getClimbedPlacesForAClimber(climber);
  if (isClimbThere){
